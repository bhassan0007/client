...
func publishNewUserEK(ctx context.Context, g *libkb.GlobalContext, merkleRoot libkb.MerkleRoot) (metadata keybase1.UserEkMetadata, err error) {
	...
	statements, err := fetchUserEKStatements(ctx, g, []keybase1.UID{g.Env.GetUID()})
	if err != nil {
		return metadata, err
	}
	var generation keybase1.EkGeneration
	statement, ok := statements[g.Env.GetUID()]
	if !ok || statement == nil {
	...
	metadata, myUserEKBoxed, err := signAndPublishUserEK(ctx, g, generation, seed, merkleRoot, statements)
	if err != nil {
		return metadata, err
	}
...
func signAndPublishUserEK(ctx context.Context, g *libkb.GlobalContext, generation keybase1.EkGeneration, seed UserEKSeed, merkleRoot libkb.MerkleRoot, prevStatements map[keybase1.UID]*keybase1.UserEkStatement) (metadata keybase1.UserEkMetadata, myUserEKBoxed *keybase1.UserEkBoxed, err error) {
